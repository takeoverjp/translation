...
この属性に対する値は、URIが指すリソースの中で鍵がどのように存在するかを表す引用文字列である。詳細は5章を参照のこと。この属性は**あってもなくてもよい**。この属性がない場合は、"identity"という値とみなされる。KEYFORMAT属性はプロトコルバージョン５から導入された。

もしメディアプレイリストがEXT-X-KEYタグを含まない場合、メディアセグメントは暗号化されていない。

鍵の形式については5章を、メディアセグメントの暗号に対する付随的な情報については5.2章、6.2.3章、6.3.6章を参照のこと。

#### 3.4.5. EXT-X-PROGRAM-DATE-TIME
EXT-X-PROGRAM-DATE-TIMEタグはメディアセグメントの最初のサンプルと、絶対日時とを関連づける。このタグは直後のメディアセグメントに対してのみ適用される。

日時の表現はISO/IEC 8601:2004 [[ISO 8601]()]に基づく。また、その表現ではタイムゾーンが指定されている**べきである**。タグの書式は下記のとおりである。
...
EXT-X-PROGRAM-DATE-TIMEタグに関するより詳しい情報は、6.2.1章と6.3.3章を参照のこと。
